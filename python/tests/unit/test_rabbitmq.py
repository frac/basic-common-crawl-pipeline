...
def test_rabbitmq_channel_publish_retry():
    ...
    with patch('bccp.rabbitmq.pika.BlockingConnection') as mock_connection_class:
        ...
        initial_failures = rabbitmq_publish_failures_counter.labels(error_type="AMQPConnectionError")._value.get()
        
        # This should eventually succeed after retry
        rabbitmq_channel_instance.basic_publish("", QUEUE_NAME, "test message")
        
        final_retries = rabbitmq_publish_retries_counter._value.get()
        final_failures = rabbitmq_publish_failures_counter.labels(error_type="AMQPConnectionError")._value.get()
...
def test_rabbitmq_channel_publish_delivery_failed():
    ...
    with patch('bccp.rabbitmq.pika.BlockingConnection') as mock_connection_class:
        ...
        initial_failures = rabbitmq_publish_failures_counter.labels(error_type="delivery_failed")._value.get()
        
        with pytest.raises(RetryError):
            rabbitmq_channel_instance.basic_publish("", QUEUE_NAME, "test message")
        
        final_failures = rabbitmq_publish_failures_counter.labels(error_type="delivery_failed")._value.get()
        assert final_failures > initial_failures
